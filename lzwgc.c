...
void lzwgc_dict_init(lzwgc_dict* dict, uint32_t size) {
    ...
    dict->alloc_next  = dyn_size - 1; // dumping ground for 'empty' token + first character
    dict->size        = size;
...
// allocate the next token-index (after using previous one)
void lzwgc_prealloc(lzwgc_dict* dict) {
...
void lzwgc_compress_recv(lzwgc_compress* st, unsigned char c) {
    ...
    for(uint32_t ii = 0; ii < ii_max; ++ii) {
        if((c == st->dict.added_char[ii]) && (s == st->dict.prev_token[ii]) // match s+c
            && (ii != st->dict.alloc_next))      // not the free token!
        {
    ...
    lzwgc_prealloc(&(st->dict)); // allocate next token
...
void lzwgc_decompress_recv(lzwgc_decompress* st, token_t rcvTok) {
    ...
    if(!valid_input_token) 
        return; // in case assertions are disabled

    // decode the token
    unsigned char * const sr = st->srbuff; // reversed string
    uint32_t ct = 0; // count of characters
    token_t  tok = rcvTok;
    ...
    unsigned char const c = (unsigned char) tok; // final token is just a character
    sr[ct++] = c; 
...
void lzwgc_decompress_recv_next(lzwgc_decompress* st) {
    ...
    unsigned char * const sr = st->srbuff; // reversed string
    uint32_t ct  = 1; // leave room at end of string (sr[0] is end).
    token_t  tok = st->prev_token;
    ...
    unsigned char const c = (unsigned char) tok; // final token is just a character
    sr[ct++] = c; // starting character (this is a reversed string)
    sr[0]    = c; // end with the starting character

    // add entry to dictionary for previous input + c.
    uint32_t const new_dict_ent = st->dict.alloc_next;
    st->dict.prev_token[new_dict_ent]  = st->prev_token;
    st->dict.added_char[new_dict_ent]  = c;
    st->dict.match_count[new_dict_ent] = 1; // obviously, we just matched it...
    st->prev_token = new_dict_ent;
    lzwgc_prealloc(&(st->dict));
